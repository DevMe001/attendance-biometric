...
showPageCount(enrollmenttRecTable, 'viewemrolledlistPrevBtn', 'viewemrolledlistNextBtn', 'viewemrolledlistPageCount', enrollRecNoTesult);
...
showPageCount(studentRecTable, 'viewmasterlistPrevBtn', 'viewmasterlistNextBtn', 'viewmasterlistPageCount', studentRecNoTesult);
...
// section by class details 
				...
				let classUnique = [];
					...
					if (listClassDetails.length > 0) {
						...
						const uniqueYearLevel = [... new Map(classUnique.map(item => [item.yearId,item])).values()];

						let sortedValueAsc = uniqueYearLevel.sort((a,b) => a.yearId > b.yearId ? 1: -1);

							sortedValueAsc.forEach((el) =>{

								console.log(el,'get element value')

									if(el.yearId == data.yearId){
											validate =true;
											gradeLevel += '<option selected value="' + el.yearId + '" >' + el.yearName + '</option>';
									}else{
											gradeLevel += '<option  value="' + el.yearId + '" >' + el.yearName + '</option>';
					...
					let getSpecificSection = [];
						...
						let sortedAscSection = getUniqueSection.sort((a,b) => a.sectionId > b.sectionId ? 1 : -1);


						console.log(sortedAscSection, 'get section');

						sortedAscSection.forEach(el => {
							

								if (el.sectionId == getSectionId) {
									validateSection = true;
									sectionList += '<option selected  value="' + el.sectionId + '" >' + el.sectionName + '</option>';
								} else {
									sectionList += '<option  value="' + el.sectionId + '" >' + el.sectionName + '</option>';
								}
						})
  ...
  $(document).ready(function () {
		$('#viewGradeLevel').on('change', function () {
			...
			console.log(selectedValue,'get selected level');
				...
				let listClassDetails = JSON.parse($('#getClassDetails').val());
					...
					if (getEnrollmentDetails.length > 0) {
						...
						console.log(sortedAscSection, 'get section list');
