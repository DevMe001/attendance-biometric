...
// resable crud operation
  ...
  {
    $selectWhere = array('account_id' => $accountId,'is_archive' => 0);
...
// list of students enrolled
  ...
  {
    return $this->get('enrollment_records', ['enrollment_id', 'lrn','fullName'], [], [], 1);
  }
...
// end teacher
  ...
  public function addClass($data){
    ...
    $where = array('section_id' => $post['classSectionName'],'year_level' => $post['classYearLevel']);
    $params = array(':section_id' => $post['classSectionName'],':year_level' => $post['classYearLevel']);
  ...
  if($getAttendance != NULL){
      foreach ($getAttendance as $attendance) {
        ...
        if ($attendance['attendance_id'] == $post['attendance_id'] && $attendance['rec_type'] == $clockTime && $dateNow == $date) {
          $isExist = true;
        }
    ...
    if($isExist){
      ...
      $clockName = $clockTime == 1 ? 'clock in' : 'clock out';
